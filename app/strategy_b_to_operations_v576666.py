...
修复点：
- 解决 MySQL 8 下 utf8mb4_0900_ai_ci 和 utf8mb4_unicode_ci 混用导致的
  Illegal mix of collations 报错。
- 所有 symbol / stock_code 字符串比较统一加 COLLATE utf8mb4_unicode_ci。
- 策略逻辑不变。
"""
...
def _maintain_old_unbought_b(cur, as_of):
    ...
    JOIN (
        ...
        JOIN (
          ...
          ON p.symbol COLLATE {COLLATE} = t.symbol COLLATE {COLLATE}
         AND DATE(p.`date`) = t.last_date
    ) px
      ON px.symbol COLLATE {COLLATE} = op.stock_code COLLATE {COLLATE}
...
def _load_candidates(cur, as_of):
    ...
    JOIN (
      ...
      ON lv.symbol COLLATE {COLLATE} = lb.symbol COLLATE {COLLATE}
...
def _load_recent_bars(cur, symbol, as_of, limit=30):
    ...
    FROM `{SRC_TABLE}`
    WHERE symbol COLLATE {COLLATE} = %s COLLATE {COLLATE}
...
def _already_exists(cur, code: str) -> bool:
    ...
    SELECT 1
    FROM `{OPS_TABLE}`
    WHERE stock_code COLLATE {COLLATE} = %s COLLATE {COLLATE}
    LIMIT 1;
    """
