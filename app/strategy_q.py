...
4) 生成两条腿的期权组合计划。
5) 可选写入 option_spreads / option_spread_legs，方便 Q 卖出机器人继续管理。

说明：这个文件由原 app.strategy_c 迁移而来。为了不让旧脚本失效，环境变量和
部分函数名仍兼容 C_* / strategy_C_*；新代码优先使用 strategy_Q_*。
"""
...
# 默认不允许策略 Q 交易保留 symbol；如果只是想用 QQQ 做计划测试，
...
# - 当前 strategy_Q_buy 仍是计划器，不拉真实期权链，所以先提供统一判断函数。
...
# Q_* 是新命名，当前先映射到旧 C_* 配置，避免迁移时实盘参数突然失效。
Q_RECORD_PLAN = C_RECORD_PLAN
Q_ENABLE_REAL_ORDER = C_ENABLE_REAL_ORDER
...
def _is_reserved_ops_symbol(code: str) -> bool:
    ...
    例如 QQQ/N 是大盘风控开关，不是策略 Q 的普通交易候选。
    """
...
def record_spread_plan(plan: SpreadPlan, allow_duplicate: bool = False) -> Optional[int]:
    ...
    try:
        if not allow_duplicate and _has_active_plan(conn, plan.underlying):
            print(f"[Q] skip record: active plan already exists for {plan.underlying}", flush=True)
            return None
...
def _upsert_c_ops_candidate(conn, row: dict) -> bool:
    ...
    if _is_reserved_ops_symbol(code):
        print(f"[Q READY] {code} skip: reserved stock_operations control row", flush=True)
        return False

    if _has_active_plan(conn, code):
        print(f"[Q READY] {code} skip: active spread exists", flush=True)
        return False
    if _has_recent_closed_spread(conn, code):
        print(f"[Q READY] {code} skip: cooldown after recent close", flush=True)
        return False
    ...
    if existing:
        ...
        if is_bought == 1:
            print(f"[Q READY] {code} skip: already bought stock_type=C", flush=True)
            return False
...
def strategy_C_refresh_candidates() -> int:
    ...
    try:
        ...
        if not buy_time_ok:
            print(f"[Q READY] skip: {buy_time_reason}", flush=True)
            return 0
        ...
        try:
            ...
            if options_bp < C_MIN_OPTIONS_BUYING_POWER:
                print(
                    f"[Q READY] skip: options_buying_power={options_bp:.2f} "
                    f"< min={C_MIN_OPTIONS_BUYING_POWER:.2f}",
                    flush=True,
                )
                return 0
        except Exception as e:
            print(f"[Q READY] skip: failed to read options buying power: {e}", flush=True)
            return 0

        active_n = _count_active_spreads(conn)
        if active_n >= C_MAX_OPEN_SPREADS:
            print(f"[Q READY] skip: active_spreads={active_n} >= max={C_MAX_OPEN_SPREADS}", flush=True)
            return 0

        active_usage = _sum_active_options_usage(conn)
        if active_usage >= C_MAX_OPTIONS_BP_USAGE:
            print(
                f"[Q READY] skip: active_options_usage={active_usage:.2f} "
                f">= cap={C_MAX_OPTIONS_BP_USAGE:.2f}",
                flush=True,
            )
            return 0
        ...
        print(
            f"[Q READY] scan_candidates={len(rows)} active={active_n} room={room} "
        ...
        for row in rows:
            ...
            try:
                ...
                if _upsert_c_ops_candidate(conn, row):
                    ready += 1
                    print(
                        f"[Q READY] {row.get('symbol')} mode={row.get('option_mode')} "
                        f"score={_safe_float(row.get('score')):.2f} "
                        f"final={_safe_float(row.get('final_score')):.2f} {reason}",
                        flush=True,
                    )
            except Exception as e:
                print(f"[Q READY] {row.get('symbol')} failed: {e}", flush=True)
                if C_DEBUG:
                    traceback.print_exc()
        return ready
    finally:
...
def print_close_plan(spread: dict, close_legs: list[dict], reason: str, metric: dict, current_value: float) -> None:
    ...
    print(
        f"[Q CLOSE PLAN] spread_id={spread.get('id')} underlying={spread.get('underlying')} "
    ...
    print(f"[Q CLOSE PLAN] reason={reason}", flush=True)
    for i, leg in enumerate(close_legs, start=1):
        print(
            f"[Q CLOSE PLAN] leg{i}: {leg['side']} {leg['cp']} "
...
def print_plan(plan: SpreadPlan) -> None:
    print(
        f"[Q PLAN] {plan.underlying} mode={plan.mode} expiry={plan.expiry} "
        f"underlying={plan.underlying_price:.2f} width={plan.width:.2f} "
        f"score={plan.signal_score:.2f}",
        flush=True,
    )
    print(f"[Q PLAN] reason={plan.signal_reason}", flush=True)
    for i, leg in enumerate(plan.legs, start=1):
        print(
            f"[Q PLAN] leg{i}: {leg.side} {leg.cp} strike={leg.strike:.2f} qty={leg.qty}",
            flush=True,
        )
...
def strategy_C_buy(code: str) -> bool:
    ...
    print(f"[Q] strategy start {code}", flush=True)
    ...
    try:
        ...
        buy_time_ok, buy_time_reason = _is_c_buy_time()
        if not buy_time_ok:
            print(f"[Q] {code} skip: {buy_time_reason}", flush=True)
            return False
        ...
        if _is_reserved_ops_symbol(code) and C_ALLOW_RESERVED_SYMBOL_TRADE != 1:
            ...
            if C_ALLOW_DIRECT_TEST == 1 and (C_ENABLE_REAL_ORDER != 1 or allow_direct_real_order):
                direct_test = True
                print(
                    f"[Q] {code} direct test mode: reserved symbol, "
                    f"will not update {OPS_TABLE}",
                    flush=True,
                )
            else:
                print(
                    f"[Q] {code} skip: reserved symbol for control row. "
        ...
        if not row and not direct_test:
            print(f"[Q] {code} skip: no C row in stock_operations", flush=True)
            return False
        if row and not direct_test and int(row.get("can_buy") or 0) != 1:
            print(f"[Q] {code} skip: can_buy={row.get('can_buy')}", flush=True)
            return False

        active_n = _count_active_spreads(conn)
        if active_n >= C_MAX_OPEN_SPREADS:
            print(f"[Q] {code} skip: active_spreads={active_n} >= max={C_MAX_OPEN_SPREADS}", flush=True)
            return False
        active_usage = _sum_active_options_usage(conn)
        if active_usage >= C_MAX_OPTIONS_BP_USAGE:
            print(
                f"[Q] {code} skip: active_options_usage={active_usage:.2f} "
                f">= cap={C_MAX_OPTIONS_BP_USAGE:.2f}",
                flush=True,
            )
            return False
        if _has_active_plan(conn, code) and not direct_test:
            print(f"[Q] {code} skip: active plan already exists", flush=True)
            if not direct_test:
                _update_ops_fields(
                    conn,
                    code,
        ...
        print(
            f"[Q] market {code}: trend={market.get('trend')} bias={market.get('bias')} "
            f"score={market.get('score')} mode={mode} price={price:.2f}",
            flush=True,
        )
        print(f"[Q] reason: {market.get('reason')}", flush=True)
        if candidate:
            print(
                f"[Q] candidate {code}: as_of={candidate.get('as_of')} "
        ...
        plan = build_spread_plan(code, mode, price, market)
        if plan is None:
            print(f"[Q] no plan built for {code}", flush=True)
            return False
        ...
        if C_ENABLE_REAL_ORDER == 1:
            try:
                ...
                print(
                    f"[Q] pricing {code}: entry={pricing.entry_price:.2f} "
                ...
                print(f"[Q] {code} real order blocked before submit: {e}", flush=True)
                if not direct_test:
                    _update_ops_fields(
                        conn,
                        code,
        ...
        if C_RECORD_PLAN == 1:
            try:
                ...
                if spread_id:
                    print(f"[Q] recorded spread_id={spread_id}", flush=True)
                ...
                print(f"[Q] record plan failed: {e}", flush=True)
                return False

        if C_ENABLE_REAL_ORDER != 1:
            print("[Q] dry-run planner only: real option order disabled", flush=True)
            return False
        ...
        try:
            ...
            print(
                f"[Q] real order submitted {code}: id={order_id} status={order_status} "
            ...
            print(f"[Q] real order submit failed {code}: {e}", flush=True)
            if spread_id:
        ...
        print(f"[Q ERROR] {code}: {e}", flush=True)
        traceback.print_exc()
        return False
    finally:
        try:
...
def strategy_C_sell(code: str) -> bool:
    ...
    """
    code = (code or "").strip().upper()
    print(f"[Q SELL] {code} spread exit manager start", flush=True)
    ...
    try:
        ...
        if not spreads:
            print(f"[Q SELL] {code} no OPEN spreads", flush=True)
            return False
        ...
        for spread in spreads:
            ...
            if not legs:
                print(f"[Q SELL] spread_id={spread_id} skip: no legs", flush=True)
                continue

            current_value = get_spread_current_value(spread, legs)
            if current_value is None:
                print(
                    f"[Q SELL] spread_id={spread_id} skip: missing current_value. "
            ...
            print(
                f"[Q SELL] spread_id={spread_id} mode={spread.get('mode')} "
            ...
            if block_same_day:
                print(f"[Q SELL] spread_id={spread_id} skip close: {block_reason}", flush=True)
                continue
            elif _is_same_day_open(spread):
                print(f"[Q SELL] spread_id={spread_id} same-day close check: {block_reason}", flush=True)
            ...
            if C_ENABLE_REAL_ORDER == 1:
                try:
                    ...
                    order_status = str(getattr(order, "status", "") or "")
                    print(
                        f"[Q SELL] real close submitted spread_id={spread_id} "
                    ...
                    print(f"[Q SELL] real close submit failed spread_id={spread_id}: {e}", flush=True)
                    _update_spread_existing_fields(
                        conn,
                        spread_id,
            ...
            if C_RECORD_PLAN == 1:
                try:
                    ...
                    print(f"[Q SELL] spread_id={spread_id} marked CLOSE_PLANNED", flush=True)
                except Exception as e:
                    print(f"[Q SELL] spread_id={spread_id} mark close planned failed: {e}", flush=True)

            planned_any = True

        if C_ENABLE_REAL_ORDER != 1:
            print("[Q SELL] dry-run close planner only: real close order disabled", flush=True)
            return False

        return planned_any

    except Exception as e:
        print(f"[Q SELL ERROR] {code}: {e}", flush=True)
        traceback.print_exc()
        return False
    finally:
        try:
...
def strategy_Q_refresh_candidates() -> int:
    """Q 候选刷新入口；兼容原 strategy_C_refresh_candidates 实现。"""
    return strategy_C_refresh_candidates()


def strategy_Q_buy(code: str) -> bool:
    """Q 期权价差开仓入口；兼容原 strategy_C_buy 实现。"""
    return strategy_C_buy(code)


def strategy_Q_sell(code: str) -> bool:
    """Q 期权价差平仓入口；兼容原 strategy_C_sell 实现。"""
    return strategy_C_sell(code)


def submit_option_combo(payload: dict) -> dict:
    """按 Q 页面选中的期权组合提交 MLEG 限价开仓单，并写入 Q 组合记录。"""
    from ultimate_v1.d_tactical import OPTION_MODES, _underlying_price
    ...
    default_cp = "C" if mode in {MODE_BULL_CALL, MODE_BEAR_CALL} else "P"
    buy_leg = OptionLeg(
        "BUY",
        str(buy.get("label") or "").split()[-1] or default_cp,
        float(buy.get("strike") or 0),
        option_symbol=str(buy.get("option_symbol") or ""),
    )
    sell_leg = OptionLeg(
        "SELL",
        str(sell.get("label") or "").split()[-1] or default_cp,
    ...
    spread_id = record_spread_plan(plan, allow_duplicate=True)
    if not spread_id:
    ...
    try:
        ...
        try:
            ...
            _update_spread_existing_fields(conn, int(spread_id), status="FAILED", close_reason=str(exc)[:500])
            raise
        order_id = str(getattr(order, "id", "") or getattr(order, "order_id", "") or "")
        order_status = str(getattr(order, "status", "") or "")
        _update_spread_existing_fields(conn, int(spread_id), order_id=order_id, status="SUBMITTED")
    finally:
        conn.close()
...
def _q_reason_filter(conn) -> tuple[str, list[str]]:
    """只筛 Q 页面手动开出的组合；兼容改名前的 D manual 记录。"""
    cols = _table_columns(conn, SPREADS_TABLE)
...
def _load_q_spreads(conn, statuses: tuple[str, ...]) -> list[dict]:
    where, params = _q_reason_filter(conn)
...
def _sync_q_submitted_spreads(conn) -> int:
    """同步 Q 开仓订单状态：只有已成交的组合才进入 OPEN，避免卖出未成交组合。"""
    rows = _load_q_spreads(conn, ("SUBMITTED",))
    if not rows:
        return 0
    ...
    for spread in rows:
        ...
        try:
            ...
            if status == "filled":
                ...
                _update_spread_existing_fields(conn, spread_id, **update)
                synced += 1
...
def _quote_q_current_value(spread: dict, legs: list[dict]) -> float | None:
    ...
    if not symbols:
        return None
    try:
        quotes = _get_option_quotes(symbols)
        net_close_cash = 0.0
        for leg in legs:
...
def q_sell_once() -> int:
    ...
    try:
        ...
        for spread in spreads:
            ...
            current_value = _quote_q_current_value(spread, legs)
            if current_value is not None:
                _update_spread_existing_fields(conn, spread_id, current_value=float(current_value))
            else:
                current_value = get_spread_current_value(spread, legs)
            if current_value is None:
            ...
            if C_ENABLE_REAL_ORDER == 1:
                try:
                    ...
                    _update_spread_existing_fields(conn, spread_id, close_reason=f"Q_CLOSE_FAIL {str(exc)[:450]}")
            elif C_RECORD_PLAN == 1:
                mark_spread_close_planned(conn, spread_id, reason, current_value, metric)
                closed += 1
...
if __name__ == "__main__":
    strategy_Q_buy(os.getenv("Q_TEST_SYMBOL", os.getenv("C_TEST_SYMBOL", "QQQ")))
